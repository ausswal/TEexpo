...
try:
    opts, args = getopt.getopt(sys.argv[1:], "hf:g:o:", [
                               "fasta=", "gff=", "output="])
...
# Open the fasta file and save the sequences in numeric order because it
# corresponds to the ID in Parent=repeat_regionID
...
# start writing in a temp output with all entries with a protein match
tempout = open(outname_temp, "w")
...
for line in gff:
    ...
    if line.startswith("###"):  # seq dividers
        new_seq = True
        continue
    # comment, only the first line will be kept because seq tag will be
    # updated in GFF
    elif line.startswith("##") and new_seq is False:
    ...
    # Deal with LTR annotation
        ...
        line = line.rstrip()
        itens = line.split("\t")
        # It will be 'protein_match' if it was annotated with LTRDigest,
        # sequences with that is what we want to keep
        feature = str(itens[2:3])
...
for entry in temp_file:
    ...
    # Get the Parent=repeat_regionID number to find the sequence in the fasta
    # file
    ...
    # number -1 to index properly (starts at 0)
    seq_id = ltr_seq[entry_number - 1]
    ...
    if seq_id not in duplicated:
        ...
        pos = str(
            ...
            better_seq_header[3] +
            "]")
        fasta_format_string = SeqRecord(
            genes_seq[seq_id], id=better_seq_id, description=pos)
    ...
    if entry_number not in entry_list:
        ...
        for line in gff:
            ...
            if re.search(
                    ...
                    description):
                # number -1 to index properly (starts at 0)
                seq_id = ltr_seq[int(entry_number) - 1]
                better_seq_header = seq_id.split("_")
                better_seq_id = str(
                    better_seq_header[0] + "_" + better_seq_header[1])
                better_entry = line
...
# Useful info
useful_info = "\nThere are in total " + \
    str(count_ltr) + " LTRretrotransposon annotated.\n"
print(useful_info)
